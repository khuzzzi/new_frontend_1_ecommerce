...
const Checkout = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://new-ecommerce-platform-backend-1.vercel.app/api/v1/productacts/gettingDataForCheckout', formData);
      if(response.data.success){
        navigate("/thanks")
        try {
          await axios.post(`https://new-ecommerce-platform-backend-1.vercel.app/api/v1/productacts/sendingemail/${formData.email}`)
        } catch (error) {
          console.log(error)
        }
      }
